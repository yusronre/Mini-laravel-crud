...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('users.create', compatct('role'));
    }
    ...
    public function rolePermission(Request $request)
    {
        ...
        //Defaultm set dua buah variable dengan nilai null
        $permissions = null;
        $hasPermission = null;
        ...
        if (!empty($role)) {
            ...
            //Query untuk mengambil  permission yang telah dimliki oleh role terkait
            $hasPermission = DB::table('role_has_permissions')
            ->select('permissions.name')
            ->join('permissions', 'role_has_permissions.permission_id', '=', 'permissions.id')
            ->where('role_id', $getRole->id)->get()->pluck('name')->all();

            //Megnabil data permission
    ...
    public function setRole(Request $request, $id)
    {
        ...
        $user = User::findOrFail($id);
        //mennggunakan syncsRoles agar terlebih dahulu menghapus semua semua role yang dimiliki
